namespace DwarfWarrior
{
    ...
    using DwarfWarrior.GameControllers;
    ...
    public class DwarfWarriorMain
    {
        ...
        private static IGameController keyboard;
        private static IRenderer renderer;
        private static IGameObjectProducer objectFactory;
        private static Engine gameEngine;

        public static void Main()
        {
            int playerInitPositionRow = ConsoleUI.CanvasRows / 2;
            int playerInitPositionCol = 0;
            int playerInitSpeedRow = 0;
            int playerInitSpeedCol = 0;

            Coordinate playerInitPosition = new Coordinate(playerInitPositionRow, playerInitPositionCol);
            Coordinate playerInitSpeed = new Coordinate(playerInitSpeedRow, playerInitSpeedCol);

            player = new Player(playerInitPosition, playerInitSpeed, ConsoleUI.PlayerBody);
            keyboard = new KeyboardController();
            renderer = new ConsoleRenderer(ConsoleUI.CanvasRows, ConsoleUI.CanvasColumns);
            objectFactory = new GameObjectsFactory();
            gameEngine = new Engine(keyboard, renderer, objectFactory, player);

            ConsoleUI.InintGameCanvas();

            BindKeyboardResponse();

            gameEngine.Run();
        }

        private static void BindKeyboardResponse()
        {
            ...
            {
                if (player.TopLeftPosition.Row < ConsoleUI.CanvasRows - player.BodyHeight)
                {
            ...
            {
                if (player.TopLeftPosition.Col < ConsoleUI.CanvasColumns - player.BodyWidth)
                {
            ...
            {
                gameEngine.ProduceShotFrom(player as ShootingObject);
            };
        }
        ...
        //    var factory = new GameObjectsFactory();
        //    var player = factory.ProduceObject(ObjectType.Player, new Coordinate(10, 10), new Coordinate(0, 0)) as Player;
        //    var engine = new Engine(keyboard, renderer, factory, player);
        //    engine.AddPlayer();

        //    engine.AddGameObject(factory.ProduceObject(ObjectType.Battlecruiser, new Coordinate(0,15), new Coordinate(0,0)));
